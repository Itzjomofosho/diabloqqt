...
local menu_elements_pois_trap =
{
    ...
    min_percentage_hits   = slider_float:new(0.1, 1.0, 0.40, get_hash(my_utility.plugin_label .. "min_percentage_hits_trap_base_pos")),
    soft_score            = slider_float:new(2.0, 15.0, 6.0, get_hash(my_utility.plugin_label .. "min_percentage_hits_trap_base_soft_core_pos")),
}
...
local spell_max_range = 3.0

local next_time_allowed_cast = 6.0;
...
local function logics(entity_list, target_selector_data, best_target)
    ...
    local keybind_can_skip = keybind_ignore_hits and keybind_used;
    ...
    local area_data = target_selector.get_most_hits_target_circular_area_heavy(player_position, 3, 4.0)
    ...
    if cast_spell.position(poison_trap_id, cast_position, 0.4)then
