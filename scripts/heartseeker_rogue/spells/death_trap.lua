...
local menu_elements_death_trap =
{
    ...
    min_hits              = slider_int:new(1, 20, 6, get_hash(my_utility.plugin_label .. "min_hits_to_casttrap_base")),
    
    allow_percentage_hits = checkbox:new(true, get_hash(my_utility.plugin_label .. "allow_percentage_hits_trap_base")),
    min_percentage_hits   = slider_float:new(0.1, 1.0, 0.40, get_hash(my_utility.plugin_label .. "min_percentage_hits_trap_base")),
...
local function logics(entity_list, target_selector_data, best_target)
    ...
    local area_data = target_selector.get_most_hits_target_circular_area_heavy(player_position, 0.2, 4.0)
    ...
    cast_spell.position(death_trap_spell_id, cast_position, 0.4);
