...
local function is_spell_allowed(spell_enable_check, next_cast_allowed_time, spell_id, cast_mode_menu)
    ...
    local current_cast_mode = cast_mode_menu
    ...
    if current_orb_mode == orb_mode.flee then
        return false
    end
...
local function is_target_within_angle(origin, reference, target, max_angle)
    local angle = reference:get_angle(target, origin);
...
local plugin_label = "LUA_EXAMPLE_NECROMANCER_BLIGHT_LUA_PLUGIN_"
