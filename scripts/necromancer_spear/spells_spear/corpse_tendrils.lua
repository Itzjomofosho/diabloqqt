...
local menu_elements = {
    ...
    main_boolean_tendrils           = checkbox:new(true, get_hash(my_utility.plugin_label .. "tendrils_boolean")),
    min_hits                        = slider_int:new(0, 30, 5, get_hash(my_utility.plugin_label .. "min_hits")),
    effect_size_affix_mult      = slider_float:new(0.0, 100.0, 0.0, get_hash(my_utility.plugin_label .. "corpse_tendrils_effect_size_affix_mult_slider_bone_spear_build")),
}
local function menu()
    
    if menu_elements.tree_tab_tendrils:push("Corpse Tendrils") then
        menu_elements.main_boolean_tendrils:render("Enable Spell", "")

        if menu_elements.main_boolean_tendrils:get() then
        menu_elements.min_hits:render("Min Hits", "")
        menu_elements.effect_size_affix_mult:render("Effect Size Affix Mult", "", 1)
        end;

       menu_elements.tree_tab_tendrils:pop()
    end 
end
...
local function logics()
    ...
    local menu_boolean = menu_elements.main_boolean_tendrils:get();
    ...
    local is_min_hit_enabled = menu_elements.min_hits:get()
    if is_min_hit_enabled then
    if best_cast_hits < menu_elements.min_hits:get() then
        return false
    end
