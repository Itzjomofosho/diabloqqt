local menu_elements = {
    ...
    enable_mouse_render = checkbox:new(false, get_hash("debug_module_enable_mouse_render_checkbox")),
}
...
local function render_menu()
    if menu_elements.main_tree:push("Debug Tools") then
        ...
        menu_elements.enable_mouse_render:render("Enable Mouse Render", "")
...
local function draw_object_information(obj, position_2d, player_position, max_distance)
    ...
    if menu_elements.show_future_position_checkbox:get() then
        draw_object_future_position(obj, player_position, max_distance, position_2d)
        position_2d.y = position_2d.y + 15
    end
...
local function render_buff()
    ...
    local top_left_position = vec2.new(0, 0)
    ...
    for i, buff in ipairs(buffs) do
        ...
        local buff_text = buff_id .. " : " .. buff_name
        local buff_position = vec2.new(0, i * 13)
        graphics.text_2d(buff_text, buff_position, 13, color_white(255))
    end

    -- enemy portion
    local enemies_list = actors_manager.get_enemy_npcs();
    local obj_top_left_position = vec2.new(400, 0)
    ...
    for i, obj in ipairs(enemies_list) do
        ...
        for i, obj_buff in ipairs(obj_buffs) do
            ...
            local obj_buff_position = vec2.new(400, i * 13)
...
local function render_mouse_info()    
    if not menu_elements.enable_mouse_render:get() then
        return
    end
    ...
    for _, actor in ipairs(actors) do
        ...
        local distance = actor_pos:squared_dist_to_ignore_z(cursor_pos)
        if distance <= radius then
            table.insert(close_actors, actor)
        end
    end

    table.sort(close_actors, function(a, b)
        return a:get_position():squared_dist_to_ignore_z(player_position) <
            b:get_position():squared_dist_to_ignore_z(player_position)
    end)

    local cursor_pos_2d = graphics.w2s(cursor_pos)
    local player_pos_2d = graphics.w2s(player_position)
    local color = color_yellow(255)
    local thickness = 2
    local closest_actor = nil
    local closest_distance = math.huge

    for _, actor in ipairs(close_actors) do
        local actor_pos = actor:get_position()
        local actor_pos_2d = graphics.w2s(actor_pos)
        if not actor_pos_2d then goto continue end
        local distance = actor_pos:squared_dist_to_ignore_z(cursor_pos)
        if distance < closest_distance and distance <= radius then
    ...
    graphics.circle_3d(cursor_pos, radius, color, thickness, 144)
    graphics.line(player_pos_2d, cursor_pos_2d, color, thickness)

    local distance = player_position:dist_to(cursor_pos)
    local center_point = vec2.new((player_pos_2d.x + cursor_pos_2d.x) / 2, (player_pos_2d.y + cursor_pos_2d.y) / 2)

    graphics.text_2d("Distance:", vec2.new(center_point.x, center_point.y - 15), 15, color)
    graphics.text_2d(tostring(distance), center_point, 15, color_white(255))

    if closest_actor then
        local is_wall_collision = target_selector.is_wall_collision(player_position, closest_actor, width)
        local is_unit_collision = target_selector.is_unit_collision(player_position, closest_actor, width)
        local collision_type = "None"
        if is_wall_collision then
            collision_type = "Wall"
            color = color_red(255)
        elseif is_unit_collision then
            collision_type = "Unit"
            color = color_red(255)
        else
            color = color_green(255)
        end
        graphics.text_2d("Collision:", vec2.new(center_point.x, center_point.y + 15), 15, color)
        graphics.text_2d(collision_type, vec2.new(center_point.x, center_point.y + 30), 15, color_white(255))

        graphics.circle_3d(cursor_pos, radius, color, thickness, 144)
        graphics.line(player_pos_2d, cursor_pos_2d, color, thickness)
        local actor_name = closest_actor:get_skin_name()
        local text_position = vec2.new(cursor_pos_2d.x + radius + 45, cursor_pos_2d.y)
        graphics.text_2d("Actor Name:", vec2.new(text_position.x, text_position.y - 15), 15, color)
        graphics.text_2d(actor_name, text_position, 15, color)
    end
end
...
local function render_visuals()
    ...
    if show_gameobjects > 0 then
        ...
        for _, obj in ipairs(game_objects) do
            ...
            if distance_to_object_sqr <= (max_distance * max_distance) and (name_filter_text == "" or string.find(object_name, name_filter_text)) then
