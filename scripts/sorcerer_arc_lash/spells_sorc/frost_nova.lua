...
local menu_elements_sorc =
{
    ...
    main_boolean       = checkbox:new(true, get_hash(my_utility.plugin_label .. "disable_enable_ability_arc")),
    min_max_targets    = slider_int:new(0, 30, 5, get_hash(my_utility.plugin_label .. "min_max_number_of_targets_for_cast_arc"))
}

local function menu()

    if menu_elements_sorc.tree_tab:push("Frost Nova") then
        menu_elements_sorc.main_boolean:render("Enable Spell", "")
 
        if menu_elements_sorc.main_boolean:get() then
            menu_elements_sorc.min_max_targets:render("Min hits", "Amount of targets to cast the spell")
        end
