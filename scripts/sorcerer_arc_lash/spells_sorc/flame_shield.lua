...
local menu_elements_sorc = 
{
    ...
    main_boolean          = checkbox:new(true, get_hash(my_utility.plugin_label .. "main_boolean_flame_shield_arc")),
    hp_usage_shield       = slider_float:new(0.0, 1.0, 0.30, get_hash(my_utility.plugin_label .. "%_in_which_shield_will_cast_arc"))
}

local function menu()
    
    if menu_elements_sorc.tree_tab:push("Flame Shield") then
        menu_elements_sorc.main_boolean:render("Enable Spell", "")

       if menu_elements_sorc.main_boolean:get() then
        menu_elements_sorc.hp_usage_shield:render("Min cast HP Percent", "", 2)
       end
...
local function logics()
    ...
    local menu_min_percentage = menu_elements_sorc.hp_usage_shield:get();
    ...
    if cast_self_spell(spell_id_flame_shield) then
        ...
        local current_time = time_since_inject();
