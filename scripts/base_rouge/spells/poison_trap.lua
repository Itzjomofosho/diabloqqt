...
local menu_elements_pois_trap =
{
    ...
    spell_range   = slider_float:new(1.0, 15.0, 2.50, get_hash(my_utility.plugin_label .. "poison_trap_spell_range")),
    spell_radius   = slider_float:new(0.50, 5.0, 3.20, get_hash(my_utility.plugin_label .. "poison_trap_spell_radius")),
}
...
local function logics(entity_list, target_selector_data, best_target)
    ...
    if not is_area_valid and not keybind_can_skip  then
        console.print("leaving 2222")
        return false;
    end

    if not area_data.main_target:is_enemy() then
        console.print("leaving 3333")
        return false;
    end
    ...
    if closest_distance_sqr > (spell_range * spell_range) and not keybind_can_skip  then
        console.print("leaving 55555 -> " .. math.sqrt(closest_distance_sqr))
        return false;
    end
