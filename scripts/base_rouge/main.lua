...
on_update(function ()
    ...
    if is_ranged then
        ...
        if current_time - global_poison_trap_last_cast_time > 1.20 and current_time - global_poison_trap_last_cast_time < 2.20 then
        ...
        if current_time - last_dash_cast_time > 6.0 then
            local nd = {}
    ...
    if spells.shadow_step.logics(best_target)then
        cast_end_time = current_time + 0.4;
        return;
    end
    ...
    if spells.caltrop.logics(closest_target)then
        cast_end_time = current_time + 0.4;
        return;
    end;
