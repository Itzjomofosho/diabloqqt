...
on_update(function ()
    ...
    if spells.lacerate.logics(target)then
        cast_end_time = current_time + 2.5;
        return;
    end;

    if spells.debilitating_roar.logics(player_position)then
        cast_end_time = current_time + 0.4;
        return;
    end;

    if spells.earthen_bulwark.logics(player_position)then
        cast_end_time = current_time + 0.4;
        return;
    end;
    ...
    if spells.poison_creeper.logics(player_position)then
        cast_end_time = current_time + 0.2;
        return;
    end;
    ...
    if spells.petrify.logics(player_position)then
        cast_end_time = current_time + 0.2;
        return;
    end;

    if spells.cataclysm.logics(player_position)then
        cast_end_time = current_time + 0.2;
        return;
    end;
